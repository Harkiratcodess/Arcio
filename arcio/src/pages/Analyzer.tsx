...
const Analyzer: React.FC = () => {
  ...
  const handleExport = () => {
    if (!analysis) {
      alert('Please analyze a repository first before exporting the report.');
      return;
    }
    const reportText = `Arcio Technical Audit Report
==================================================
...
Scores Breakdown:
- Code Quality: ${analysis.scores.codeQuality}/100
- Architecture: ${analysis.scores.architecture}/100
- Naming Patterns: ${analysis.scores.naming}/100
- Documentation: ${analysis.scores.readme}/100

Key Metrics:
- Maintainability: ${analysis.metrics.maintainability.score}%
- Complexity Risk: ${analysis.metrics.complexity.score}%
- Security Health: ${analysis.metrics.security.score}%

Critical Issues Detected:
${analysis.structureIssues.map((issue, idx) => `${idx + 1}. ${issue}`).join('\n') || 'None detected.'}

Proposed Improvements:
${analysis.improvements.map((imp, idx) => `${idx + 1}. [${imp.difficulty}] ${imp.title}: ${imp.description}`).join('\n') || 'None proposed.'}

--------------------------------------------------
Generated by Arcio Analyzer (Beta v0.1)`;

    const element = document.createElement("a");
    const file = new Blob([reportText], {type: 'text/plain'});
    element.href = URL.createObjectURL(file);
    element.download = `${analysis.repo.name}_arcio_audit.txt`;
          ...
          <div className="premium-card p-8 bg-white border border-stone-200">
            ...
            <div className="space-y-6">
              ...
              <button 
                ...
                className="w-full py-4 bg-teal-800 hover:bg-teal-900 text-white text-[11px] font-black uppercase tracking-[0.2em] rounded-xl transition-all active:scale-95 shadow-lg shadow-teal-900/10 disabled:opacity-50 flex items-center justify-center gap-2"
              >
                {loading && (
                  <svg className="w-4.5 h-4.5 animate-spin text-white" viewBox="0 0 24 24" fill="none">
          ...
          <div className="flex items-center justify-between">
            ...
            <div className="flex gap-4">
              {usage?.upgradeAvailable && (
                <button 
                  onClick={() => alert("Pro plans are free and fully unlocked during our beta period!")}
                  className="px-5 py-2 bg-stone-900 text-white text-[10px] font-black uppercase tracking-widest rounded-lg hover:bg-stone-800 transition-all shadow-lg shadow-stone-900/10 cursor-pointer"
                >
                  Upgrade to Pro
                </button>
              )}
              <button 
                ...
                className="flex items-center gap-2 px-4 py-2 border border-stone-200 rounded-lg text-[10px] font-black uppercase tracking-widest text-stone-600 hover:bg-stone-50 transition-all cursor-pointer"
              >
