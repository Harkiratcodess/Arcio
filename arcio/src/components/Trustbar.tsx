const trustItems = [
  ...
  },
  {
    ...
    title: "Read-only GitHub",
    subtitle: "we never write",
  },
  {
    icon: (
      <svg className="w-4 h-4" fill="none" viewBox="0 0 24 24" strokeWidth={1.5} stroke="currentColor">
    ...
    title: "Founder-built",
    subtitle: "feedback shapes v1",
  },
];

const TrustBar = () => {
  return (
    <div className="w-full bg-[#EAE7DF] border-y border-stone-200">
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 divide-x divide-stone-200">
          {trustItems.map((item, i) => (
            <div key={i} className="flex items-center gap-3 px-6 py-5">
              ...
              <div>
                <p className="text-[13px] font-semibold text-stone-800">{item.title}</p>
                <p className="text-[11px] text-stone-400 font-mono mt-0.5">{item.subtitle}</p>
              </div>
            </div>
          ))}
        </div>
